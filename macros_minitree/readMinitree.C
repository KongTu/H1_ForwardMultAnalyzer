...
void readMinitree(const int ifile_ = 0, const bool isReweigh = false){
	...
	TFile* file = 0;
	if(ifile_ == 0){
		file = new TFile("../new_output/data_highE_0607_noReweight_Tree_hadCaliNewLooseTrack.root");
	}else if(ifile_ == 1) {
		if(!isReweigh) file = new TFile("../new_output/mc_highE_DJANGOH_noReweight_Tree_hadCaliNew.root");
		else file = new TFile("../new_output/mc_highE_DJANGOH_fullReweight_Tree_hadCaliNewLooseTrack.root");
	}else if(ifile_ == 2){
		if(!isReweigh) file = new TFile("../new_output/mc_highE_RAPGAP_noReweight_Tree_hadCaliNew.root");
		else file = new TFile("../new_output/mc_highE_RAPGAP_fullReweight_Tree_hadCaliNewLooseTrack.root");
	}
	//output files
	TString outname;
	if( ifile_ == 0 ){
		outname = "../minitree_output/Pn_hist_data_hadCaliNew_extendEtalabLooseTrack.root";
	}else if( ifile_ == 1 ){
		if(!isReweigh) outname = "../minitree_output/Pn_hist_django_extendEtalabLooseTrack.root";
		else outname = "../minitree_output/Pn_hist_django_extendEtalabLooseTrack_reweigh.root";
	}else if( ifile_ == 2 ){
		if(!isReweigh) outname = "../minitree_output/Pn_hist_rapgap_extendEtalabLooseTrack.root";
		else outname = "../minitree_output/Pn_hist_rapgap_extendEtalabLooseTrack_reweigh.root";
	}
