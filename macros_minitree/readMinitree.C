...
void readMinitree(const int ifile_ = 0, const bool isReweigh = false){
	...
	if(ifile_ == 0){
		...
		else file = new TFile("../new_output/mc_highE_RAPGAP_fullReweight_Tree_hadCaliNewKine_checkWeight.root");
	}
	else if(ifile_ == 3){
	...
	if( ifile_ == 0 ){
		...
		else outname = "../minitree_output/Pn_hist_rapgap_hadCaliNewKine_reweigh_checkWeight-June30.root";
	}
	else if( ifile_ == 3 ){
