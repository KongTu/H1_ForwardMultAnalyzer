	...
	for(int iQ2=0;iQ2<4;iQ2++){
		for(int iy=0;iy<4;iy++){
			for(int m=0;m<4;m++){
				 ...
				 h_dedxElectronSinThetaCut[iQ2][iy][m] = new TH1D(Form("h_dedxElectronSinThetaCut_%d_%d_%d",iQ2,iy,m),";1/sin(#theta)",100,-10,10);
	...
	for(int ievent = 0; ievent < tree->GetEntries(); ievent++){
	   ...
	   //setting event Q2 and y indices
		...
		for(int itrk = 0; itrk < nRECtrack_mini; itrk++){
			...
			//double nested loops
			TLorentzVector k0s_candidate(-99,-99,-99,-99);
			TLorentzVector pip(-99,-99,-99,-99);
			TLorentzVector pim(-99,-99,-99,-99);
			for(int jtrk = itrk+1; jtrk < nRECtrack_mini; jtrk++){
				...
				if(Q2_INDEX>-1 && y_INDEX>-1 && k0s_candidate.E()!=-99) {h_K0sMass[Q2_INDEX][y_INDEX]->Fill( k0s_candidate.M(), w_mini );}
