			...
			if(Q2_INDEX>=0 && y_INDEX >= 0 && isQEDcMC_mini >= 0) {
				...
				for(int itype=0;itype<3;itype++){
					...
					if( (ifile_==1 && ievent<3e7) && isQEDcMC_mini==0 ){
						...
						if( eMC.Theta()*TMath::RadToDeg()>179.8 || eMC.Theta()*TMath::RadToDeg()<3.6 ) passcut = 0;
						if( eGamma.E() < 0.5 ) passcut = 0;
						if( eGamma.Theta()*TMath::RadToDeg()>179.8 || eGamma.Theta()*TMath::RadToDeg()<3.6 ) passcut = 0;
