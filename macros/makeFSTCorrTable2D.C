...
void makeFSTCorrTable2D() {
      ...
      for(int i=0;i<tree->GetEntries();i++) {
         ...
         for(int k=0;k<nMCtrack;k++) {
            ...
            if(irec>=0) {
               ...
               double phiREC = TMath::ATan(pyMC[irec]/pxMC[irec]);
               if( pxREC[irec] < 0 && pyREC[irec] > 0 ) phiREC = phiREC+3.14;
               if( pxREC[irec] < 0 && pyREC[irec] < 0 ) phiREC = phiREC-3.14;
               if(phiREC < phiMin || phiREC > phiMax ) continue;
         ...
         for(int j = 0; j<nRECtrack; j++){
            ...
            if(jrec<0) {
               if(type==4)h_recMatchEtaStar->Fill(eta,pt,w);
            }
         }
   ...
   for(int i = 0; i < h_genMatchEtaStar->GetNbinsX(); i++){
      for(int j = 0; j < h_genMatchEtaStar->GetNbinsY(); j++){
         ...
         if( correction != 0. && i == 10 && j == 10) cout << "corr " << correction << endl;
   ...
   corr->GetYaxis()->SetRangeUser(0,5.0);

   corr->GetXaxis()->SetTitleOffset(1.8);
   corr->GetXaxis()->SetRangeUser(-2.5,0.2);
   
   corr->GetZaxis()->SetTitleOffset(1.6);
   corr->GetZaxis()->SetTitle("1/(eff*(1-fak))");
   corr->Draw("lego2");

   c1->Print("../figures/Final/FSTCorrectionTable.pdf");
